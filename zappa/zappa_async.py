...
```
   { "command": "zappa.zappa_async.route_lambda_task",
...
5. The new lambda instance is invoked with the message above,
   ...
   dispatches the full message to zappa.zappa_async.route_lambda_task, which
...
class LambdaAsyncResponse(object):
    ...
    def _send(self, message):
        ...
        """
        message['command'] = 'zappa.zappa_async.route_lambda_task'
...
class SnsAsyncResponse(LambdaAsyncResponse):
    ...
    def _send(self, message):
        ...
        """
        message['command'] = 'zappa.zappa_async.route_sns_task'
        payload = json.dumps(message).encode('utf-8')
...
def run(func, args=[], kwargs={}, service='lambda', capture_response=False,
    ...
    import zappa.zappa_async.run
    zappa.zappa_async.run(func, args, kwargs)

    If you want to use SNS, then do:

    zappa.zappa_async.run(func, args, kwargs, service='sns')
