...
For example, if you have a Flask API for ordering a pie, you can call your `bake` function seamlessly in a completely separate Lambda instance by using the `zappa.zappa_async.task` decorator like so:

```python
from flask import Flask
from zappa.zappa_async import task
app = Flask(__name__)
...
When calls to @task decorated functions or the zappa.zappa_async.run command occur outside of Lambda, such as your local dev environment,
...
```python
from zappa.zappa_async import task_sns
@task_sns
```
...
You can also use this functionality without a decorator by passing your function to `zappa.zappa_async.run`, like so:

```python
from zappa.zappa_async import run
...
If those task() parameters were not used, then EC2 would execute the function locally. These same
 `remote_aws_lambda_function_name` and `remote_aws_region` arguments can be used on the zappa.zappa_async.run() function as well.
...
```python
from zappa.zappa_async import task, get_async_response
